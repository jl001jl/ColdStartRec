...
class DIERM(BaseModel):
    def __init__(self,
                 ...
                 net_dropout=0,
                 batch_norm=False,
                 dierm_target_field=[("user_id",)],
                 dierm_sequence_field=[("user_has_attend_event",)],
                 din_use_softmax=False,
                 embedding_regularizer=None,
                 net_regularizer=None,
                 **kwargs):
        super(DIERM, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
                                    embedding_regularizer=embedding_regularizer,
                                    net_regularizer=net_regularizer,
                                    **kwargs)
        if not isinstance(dierm_target_field, list):
            dierm_target_field = [dierm_target_field]
        self.dierm_target_field = dierm_target_field
        if not isinstance(dierm_sequence_field, list):
            dierm_sequence_field = [dierm_sequence_field]
        self.dierm_sequence_field = dierm_sequence_field
        assert len(self.dierm_target_field) == len(self.dierm_sequence_field), \
        ...
        self.attention_layers = nn.ModuleList(
            [DIERM_Attention(embedding_dim * len(target_field) if type(target_field) == tuple \
                                                             else embedding_dim,
             ...
             for target_field in self.dierm_target_field])
    ...
    def forward(self, inputs):
        ...
        for idx, (target_field, sequence_field) in enumerate(zip(self.dierm_target_field,
                                                                 self.dierm_sequence_field)):
            ...
            seq_field = np.array([sequence_field]).flatten()[0]
...
class DIERM_Attention(nn.Module):
    def __init__(self,
                 embedding_dim=64,
                 attention_units=[32],
                 ...
                 batch_norm=False,
                 use_softmax=False):
        super(DIERM_Attention, self).__init__()
        self.embedding_dim = embedding_dim
